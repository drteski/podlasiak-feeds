...
const furdoszobaplusHuFeed = async (data, language, options) => {
	const products = prepareProducts(data, options).map((products) => {
		...
		products.forEach((product) => {
			...
			const productDetails = (attributes[language].length === undefined ? [attributes[language]] : attributes[language]).filter((attribute) => {
				if (attribute.name !== 'Kézbesítési információ') return attribute;
			});
...
const furdoszobaplusHuFeedStockUpdate = async (data, language, options) => {
	const products = prepareProducts(data, options).map((products) => {
		const chunk = [];
		products.forEach((product) => {
