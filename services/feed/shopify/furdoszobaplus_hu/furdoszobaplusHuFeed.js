...
const furdoszobaplusHuFeed = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], activeProducts = true, activeVariants = true, minStock, chunks = 0 }) => {
	const products = productsChunker(
		...
		products.forEach((product) => {
			const { title, sku, stock, weight, ean, producer, attributes, variantName, category, images, sellPrice } = product;

			if (producer === 'Toolight' || producer === 'Tutumi' || producer === 'Hadwao' || producer === 'Calani' || producer === 'Bluegarden') return;
			...
			chunk.push({
				'Handle': slugify(titleVariant.toLowerCase()),
				'Command': 'NEW',
				'Status': 'Active',
				'Vendor': producer,
				'Title': titleVariant,
...
const furdoszobaplusHuFeedStockUpdate = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], minStock, chunks = 0 }) => {
	const products = productsChunker(
		productsFilter(aliasesFilter(data, aliases), {
...
export const generateFurdoszobaplusHuFeed = async (products, config) => {
	return new Promise(async (resolve) => {
		const shouldRun = await runFeedGenerator(config.name);
		if (!shouldRun) return resolve();
		for await (const language of config.languages) {
			await furdoszobaplusHuFeed(products, language, config).then(async (data) => {
				await saveFeedFileToDisk(data, 'furdoszobaplus', 'csv', '../generate/feed/', ',', true);
			});
			await furdoszobaplusHuFeedStockUpdate(products, language, config).then(async (data) => {
				await saveFeedFileToDisk(data, 'furdoszobaplus_update', 'csv', '../generate/feed/', ',', true);
			});
		}
