...
const baiehomeRoFeed = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], activeProducts = true, activeVariants = true, minStock, chunks = 0 }) => {
...
const baiehomeRoFeedStockUpdate = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], minStock, chunks = 0 }) => {
	const products = productsChunker(
		productsFilter(aliasesFilter(data, aliases), {
...
export const generateBaiehomeRoFeed = async (products, config) => {
	return new Promise(async (resolve) => {
		const shouldRun = await runFeedGenerator(config.name);
		if (!shouldRun) return resolve();
		for await (const language of config.languages) {
			await baiehomeRoFeed(products, language, config).then(async (data) => {
				await saveFeedFileToDisk(data, 'baiehome', 'csv', '../generate/feed/', ',', true);
			});
			await baiehomeRoFeedStockUpdate(products, language, config).then(async (data) => {
				await saveFeedFileToDisk(data, 'baiehome_update', 'csv', '../generate/feed/', ',', true);
			});
		}
