...
const baiehomeRoFeed = async (data, language, options) => {
	const products = prepareProducts(data, options).map((products) => {
		...
		products.forEach((product) => {
			...
			const productDetails = (attributes[language].length === undefined ? [attributes[language]] : attributes[language]).filter((attribute) => {
				if (attribute.name !== 'Informații de livrare') return attribute;
			});
...
const baiehomeRoFeedStockUpdate = async (data, language, options) => {
	const products = prepareProducts(data, options).map((products) => {
		const chunk = [];
		products.forEach((product) => {
