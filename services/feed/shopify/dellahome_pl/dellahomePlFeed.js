...
const dellahomePlFeed = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], activeProducts = true, activeVariants = true, minStock, chunks = 0 }) => {
...
const dellahomePlFeedStockUpdate = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], minStock, chunks = 0 }) => {
	const products = productsChunker(
		productsFilter(aliasesFilter(data, aliases), {
...
export const generateDellahomePlFeed = async (products, config) => {
	return new Promise(async (resolve) => {
		const shouldRun = await runFeedGenerator(config.name);
		if (!shouldRun) return resolve();
		for await (const language of config.languages) {
			await dellahomePlFeed(products, language, config).then(async (data) => {
				await saveFeedFileToDisk(data, 'dellahome', 'csv', '../generate/feed/', ',', true);
			});
			await dellahomePlFeedStockUpdate(products, language, config).then(async (data) => {
