...
const badezimmerperleDeFeed = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], activeProducts = true, activeVariants = true, minStock, chunks = 0 }) => {
...
const badezimmerperleDeFeedStockUpdate = async (data, language, { mu = 0, aliases = ['Rea', 'Tutumi', 'Toolight'], minStock, chunks = 0 }) => {
	const products = productsChunker(
		productsFilter(aliasesFilter(data, aliases), {
...
export const generateBadezimmerperleDeFeed = async (products, config) => {
	return new Promise(async (resolve) => {
		const shouldRun = await runFeedGenerator(config.name);
		if (!shouldRun) return resolve();
		for await (const language of config.languages) {
			await badezimmerperleDeFeed(products, language, config).then(async (data) => {
				await saveFeedFileToDisk(data, 'badezimmerperle', 'csv', '../generate/feed/', ',', true);
			});
			await badezimmerperleDeFeedStockUpdate(products, language, config).then(async (data) => {
				await saveFeedFileToDisk(data, 'badezimmerperle_update', 'csv', '../generate/feed/', ',', true);
			});
		}
