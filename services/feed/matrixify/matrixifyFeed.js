...
import { addMuToPrice, aliasesFilter, productsChunker, saveFeedFileToDisk } from '../../processFeed.js';
	...
	const products = productsChunker(totalProducts, chunks).map((prods) => {
		...
		prods.forEach((product) => {
			...
			chunk.push({
				...
				'Variant Price': addMuToPrice(sellPrice[language].price, mu) * 0.95,
				'Variant Compare At Price': addMuToPrice(sellPrice[language].price, mu) * 0.95,
				'Variant Requires Shipping': 'TRUE',
				'Variant Taxable': 'TRUE',
				'Variant Barcode': ean,
				'Variant Weight Unit': 'kg',
				'Image Src': imagesUrl(images, language, producer, 'default')[0],
				'Image Command': 'MERGE',
				'Image Position': 1,
				...
				'Metafield: custom.second_image [single_line_text_field]':
					imagesUrl(images, language, producer, 'default')[1] === undefined
						? ''
						: `<img style="height: 360px; width: auto; object-fit: contain; object-position: center;" src="${imagesUrl(images, language, producer, 'default')[1]}" alt="">`,
				'Metafield: custom.third_image [single_line_text_field]':
					imagesUrl(images, language, producer, 'default')[2] === undefined
						? ''
						: `<img style="height: 360px; width: auto; object-fit: contain; object-position: center;" src="${imagesUrl(images, language, producer, 'default')[2]} " alt="">`,
			});
			imagesUrl(images, language, producer, 'default').forEach((image, index) => {
				if (index === 0) return;
				chunk.push({
					Handle:
						language === 'ro' || language === 'fr' || language === 'hu'
