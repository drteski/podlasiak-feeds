...
const generateCandeluxCsv = async () => {
	const feedData = new Promise(async (resolve, reject) => {
		const stream = await fs.createReadStream(
			'../public/temp/feed.xml',
			'utf8'
		);
		const data = await axios
			.get('https://lazienka-rea.com.pl/feed/generate/full_offer', {
				timeout: 0,
				responseType: 'stream',
			})
			.then((res) => res.data)
		...
		const xmlFileWriteStream = new XmlStream(stream);
	...
	feed.products.forEach((product) => {
		...
		const cenaPodstawowaNowa = baseTaxonomy.map((base) => {
			...
			return {
				id: base.id,
				price: cenaPodstawowa[priceIndex].gross.replace('.', '|'),
				name: base.name,
			};
		});
		...
		const cenaPromocyjnaNowa = saleTaxonomy.map((base) => {
			...
			return {
				id: base.id,
				price: cenaPromocyjna[priceIndex].gross.replace('.', '|'),
				name: base.name,
			};
		});
	...
	console.log(`ceny-grecja.csv zapisany.`);
};
