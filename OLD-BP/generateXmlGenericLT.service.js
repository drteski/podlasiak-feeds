...
const feedBuilder = {
	...
	async feedBuliderGenericLTXML(obj) {
		const root = builder
			.create({
				version: '1.0',
				encoding: 'UTF-8',
	...
	async generateFeedGenericLTXML() {
		const data = await this.prepareData();
		const xmlMall = await feedBuilder.feedBuliderGenericLTXML(data);
		await this.saveFileToDisk(xmlMall, 'generic_lt');
		console.log(`generic_lt.xml zapisany.`);
	},
};

const generateGenericLTXMLFeed = async () => {
	try {
		await feedBuilder.generateFeedGenericLTXML();
	} catch (e) {
		console.log(e);
	}
};
export default generateGenericLTXMLFeed;
