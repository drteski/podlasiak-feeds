...
const feedBuilder = {
	...
	async feedBuliderCenejeToolight(obj) {
		const root = builder
	...
	async generateFeedCenejeToolight() {
		const data = await this.prepareData();
		const xmlCenejeToolight = await feedBuilder.feedBuliderCenejeToolight(
			data
		);
		await this.saveFileToDisk(xmlCenejeToolight, 'ceneje_toolight');
		console.log(`ceneje_toolight.xml zapisany.`);
	},
};

const generateCenejeToolight = async () => {
	try {
		await feedBuilder.generateFeedCenejeToolight();
	} catch (e) {
		console.log(e);
	}
};

export default generateCenejeToolight;
