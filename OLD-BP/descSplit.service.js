...
import { replaceEntities } from '../services/products/utils/replaceEntities.js';
...
const generateJanekCsv = async () => {
	...
	parsedFile.forEach((data) => {
		const tag = replaceEntities(data['Metafield: description_tag [string]']);
		const first = replaceEntities(data['Metafield: custom']['first_description_row '].single_line_text_field);
		const second = replaceEntities(data['Metafield: custom']['second_description_row '].single_line_text_field);
		fileToSave.push({
			'Handle': data.Handle,
			'Body HTML': data['Body HTML'],
			'Vendor': data.Vendor,
			'Metafield: description_tag [string]': tag,
			'Metafield: custom.first_description_row [single_line_text_field]': first,
			'Metafield: custom.second_description_row [single_line_text_field]': second,
		});
	});
